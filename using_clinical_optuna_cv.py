...
class CFG:
    ...
    posterior_txt_drop_p: float = 0.2
    ...
    fuse_x_proj_dim: int = 64
    fuse_x_detach: bool = False
    fuse_x_drop_p: float = 0.2
    ...
    # using_clinical: text time-decay (lambda) for sequential tasks
    ...
    seed: int = 123
    epochs: int = 30  # (outer_epochs alias; stage-specific epochs are set internally)
    inner_epochs: int = 10
    outer_epochs: int = 30
    patience: int = 3
    lr: float = 1e-3
    weight_decay: float = 1e-4
    grad_clip: float = 1.0  # 0 => no clip
...
# =========================
# UsingClinical baseline model (Khadanga et al., 2019 스타일)
#   - Time-series LSTM
#   - Text feature z_t with exponential time-decay over past notes
#   - Logit_t = W_h h_t + W_z z_t + b
# =========================
class UsingClinicalModel(nn.Module):
    def __init__(self, Dx: int, Du: int, De: int, Dt: int, Ds: int, cfg: CFG, meta_bags: Optional[MetaBagsTorch] = None):
        super().__init__()
        self.cfg = cfg
        self.Dx, self.Du, self.De, self.Dt, self.Ds = int(Dx), int(Du), int(De), int(Dt), int(Ds)
        self.use_text = bool(getattr(cfg, 'use_text', True)) and self.Dt > 0

        # time-series input: [x, x_mask, u, e] (+ optional static repeated)
        self.use_static_in_ts = bool(getattr(cfg, 'death_use_static', False))  # reuse flag; default False recommended for faithful baseline
        ts_in = self.Dx + self.Dx + self.Du + self.De + (self.Ds if self.use_static_in_ts else 0)

        ts_hidden = int(getattr(cfg, 'enc_hidden', 64))
        ts_layers = int(getattr(cfg, 'enc_layers', 1)) if hasattr(cfg, 'enc_layers') else 1
        ts_dropout = float(getattr(cfg, 'dropout', 0.0)) if ts_layers > 1 else 0.0
        ...
        # text projection (already numeric txt__ features per timestep) -> z_i
        txt_hidden = int(getattr(cfg, 'txt_hidden', 64))
        self.txt_hidden = txt_hidden
        if self.use_text:
            self.txt_proj = nn.Sequential(
                nn.Linear(self.Dt, txt_hidden),
                nn.ReLU(),
        ...
        # head: W_h h_t + W_z z_t + b
        self.W_h = nn.Linear(ts_hidden, 1, bias=False)
        self.W_z = nn.Linear(txt_hidden, 1, bias=False) if self.use_text else None
        self.b = nn.Parameter(torch.zeros(1))

    @staticmethod
    def _make_txt_present(txt: torch.Tensor) -> torch.Tensor:
        if txt.size(-1) > 0:
            return (txt.abs().sum(dim=-1, keepdim=True) > 0).float()
        return txt.new_zeros((txt.size(0), txt.size(1), 1))

    def _exp_decay_aggregate(self, z_raw: torch.Tensor, present: torch.Tensor) -> torch.Tensor:
        """Compute z_t = (sum_{i<=t} exp(-lambda*(t-i)) z_i) / (sum_{i<=t} exp(-lambda*(t-i)) 1_i)."""
        lam = float(getattr(self.cfg, 'decay_lambda', 0.01) or 0.01)
        # stable: decay = exp(-lam)
        decay = float(math.exp(-lam))
        B, T, H = z_raw.shape
        v = z_raw.new_zeros((B, H))
        s = z_raw.new_zeros((B, 1))
        out = []
        for t in range(T):
            v = v * decay + z_raw[:, t, :]
            s = s * decay + present[:, t, :]
            zt = v / (s.clamp_min(1e-6))
            # if no note seen so far: set zero
            zt = zt * (s > 0).float()
            out.append(zt.unsqueeze(1))
        return torch.cat(out, dim=1)

    def forward_logits(self, batch: Dict[str, torch.Tensor], sample_latent: bool = False):
        x = batch['x']
        xm = batch['x_mask']
        u = batch['u']
        e = batch['e']
        ...
        if self.use_static_in_ts and s is not None and s.numel() > 0:
            s_rep = s.unsqueeze(1).expand(-1, x.size(1), -1)
            ts_inp = torch.cat([x, xm, u, e, s_rep], dim=-1)
        else:
            ts_inp = torch.cat([x, xm, u, e], dim=-1)

        h, _ = self.ts_lstm(ts_inp)

        if self.use_text:
            txt = batch['txt']
            present = self._make_txt_present(txt)
            z_raw = self.txt_proj(txt)
            z_raw = z_raw * present  # absent -> 0
            z_t = self._exp_decay_aggregate(z_raw, present)
            logits = self.W_h(h) + self.W_z(z_t) + self.b
        else:
            logits = self.W_h(h) + self.b

        logits = logits.squeeze(-1)
        return logits, None, None

    def forward(self, batch: Dict[str, torch.Tensor], bce_loss_fn: nn.Module, sample_latent: bool = False):
        logits, _, _ = self.forward_logits(batch, sample_latent=sample_latent)
        y = batch['y']
        m = (batch['seq_mask'] * batch['time_mask'] * batch['y_mask']).float()
        loss = bce_loss_fn(logits, y)
        loss = (loss * m).sum() / m.sum().clamp_min(1.0)
        # y_valid is used by some logging code (denom). Keep for compatibility.
        y_valid = m
        return {'loss': loss, 'logits': logits, 'y_valid': y_valid}


# -------------------------
# Training (UsingClinical: classification-only)
# -------------------------
def train_on_split_usingclinical(
    cfg: CFG,
    store_tr: ArrayStore,
    stays_train: np.ndarray,
    store_va: ArrayStore,
    ...
    model = UsingClinicalModel(Dx=Dx, Du=Du, De=De, Dt=Dt, Ds=Ds, cfg=cfg, meta_bags=meta_bags).to(device)
    ...
    for ep in range(1, cfg.epochs + 1):
        ...
        for batch in ld_tr:
            ...
            opt.zero_grad(set_to_none=True)
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                out = model.forward(batch, bce_loss_fn=bce_loss_fn, sample_latent=False)
                loss = out['loss']
        ...
        with torch.no_grad():
            for batch in ld_va:
                ...
                with torch.cuda.amp.autocast(enabled=amp_enabled):
                    ...
                    m = (batch['seq_mask'] * batch['time_mask'] * batch['y_mask']).float()
                    loss = bce_loss_fn(logits, y)
                    va_loss_num += float((loss * m).sum().detach().cpu().item())
                    va_loss_den += float(m.sum().detach().cpu().item())

        va_loss = va_loss_num / max(1.0, va_loss_den)

        print(
            f"[{tag}] Ep {ep:03d} | train_bce={tr_loss:.4f} | val_bce={va_loss:.4f} | best_val={best_val_loss:.4f} | pos_w={posw_used:.2f} | decay_lambda={float(getattr(cfg,'decay_lambda',0.01)):.4g}"
        )
...
def apply_trial_params(cfg: CFG, params: Dict[str, Any]) -> None:
    """
    Apply a dict of parameters (e.g., Optuna best_params) to cfg.

    - Robust to missing keys (older studies/json may not contain newly added params).
    - lambda_death is FIXED to 1.0 regardless of params/CLI.
    """
    ...
    cfg.encoder_txt_drop_p = _get("encoder_txt_drop_p", float, cfg.encoder_txt_drop_p)
    ...
    cfg.grad_clip = _get("grad_clip", float, cfg.grad_clip)

    cfg.z_dim = _get("z_dim", int, cfg.z_dim)
    cfg.s_emb_dim = _get("s_emb_dim", int, cfg.s_emb_dim)
    cfg.enc_hidden = _get("enc_hidden", int, cfg.enc_hidden)
    cfg.trans_hidden = _get("trans_hidden", int, cfg.trans_hidden)
    cfg.dec_hidden = _get("dec_hidden", int, cfg.dec_hidden)

    cfg.dropout = _get("dropout", float, cfg.dropout)

    cfg.decay_lambda = _get("decay_lambda", float, getattr(cfg, "decay_lambda", 0.01))

    cfg.txt_hidden = _get("txt_hidden", int, cfg.txt_hidden)
    cfg.txt_layers = _get("txt_layers", int, getattr(cfg, "txt_layers", 2))
    cfg.meta_emb_dim = _get("meta_emb_dim", int, getattr(cfg, "meta_emb_dim", 8))

    # FIXED (do not tune)
    cfg.lambda_death = 1.0

    cfg.beta_kl = _get("beta_kl", float, cfg.beta_kl)
    cfg.lambda_distill = _get("lambda_distill", float, cfg.lambda_distill)

    cfg.pos_weight_mult = _get("pos_weight_mult", float, cfg.pos_weight_mult)
...
def suggest_trial_params(trial):
    ...
    # time-series LSTM hidden (paper는 64 units 사용)
    hidden = trial.suggest_int("enc_hidden", 32, 256, step=32)
    params["enc_hidden"] = hidden

    # text projection hidden (paper는 64-dimensional text rep 사용)
    params["txt_hidden"] = trial.suggest_int("txt_hidden", 32, 256, step=32)

    # exponential time-decay lambda (paper best: 0.01)
    params["decay_lambda"] = trial.suggest_float("decay_lambda", 1e-3, 1e-1, log=True)

    # optional
    params["grad_clip"] = 2.0
    params["pos_weight_mult"] = 0.5
...
def _inner_fold_worker(payload: Dict[str, Any]) -> float:
    ...
    # stage-specific epochs: inner-fold (Optuna)
    if getattr(cfg_k, 'inner_epochs', None) is not None:
        try:
            ie = int(getattr(cfg_k, 'inner_epochs'))
            if ie > 0:
                cfg_k.epochs = ie
        except Exception:
            pass
    ...
    os.makedirs(out_dir, exist_ok=True)
    res = train_on_split_usingclinical(
        cfg=cfg_k,
        store_tr=store_pool,
...
def run_outer_fold_optuna_nested(fold_dir: str, cfg: CFG) -> Dict[str, Any]:
    ...
    # ---- optuna study ----
    study_name = f"fold{fold_id}_nested"
    sampler = None
    ...
    storage = None
    storage_path = os.path.join(out_fold, "optuna_study.db")
    try:
        ...
        if cfg_trial.inner_parallel:
            ...
            with ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as ex:
                for k, (tr_idx, va_idx) in enumerate(splits):
                    ...
                    out_trial = os.path.join(out_fold, "optuna", f"trial_{trial.number:05d}", f"inner_{k}")
            ...
            for k, (tr_idx, va_idx) in enumerate(skf.split(stays_pool, y_pool)):
                ...
                # stage-specific epochs: inner-fold (Optuna)
                try:
                    ie = int(getattr(cfg_k, 'inner_epochs', cfg_k.epochs))
                    if ie > 0:
                        cfg_k.epochs = ie
                except Exception:
                    pass
    ...
    with open(os.path.join(out_fold, "optuna_best_params.json"), "w") as f:
        json.dump(best_params, f, indent=2)

    print(f"[FOLD {fold_id}] ✅ Optuna best value={best.value:.4f} | params saved -> optuna_best_params.json")
    ...
    # stage-specific epochs: outer-fold final refit
    try:
        oe = int(getattr(cfg_best, 'outer_epochs', cfg_best.epochs))
        if oe > 0:
            cfg_best.epochs = oe
    except Exception:
        pass

    res_final = train_on_split_usingclinical(
        cfg=cfg_best,
        store_tr=store_pool,
    ...
    # (using_clinical workflow) do not override cfg_loaded flags here
    ...
    model = UsingClinicalModel(Dx=Dx, Du=Du, De=De, Dt=Dt, Ds=Ds, cfg=cfg_loaded, meta_bags=meta_bags_eval).to(cfg_loaded.device)
    model.load_state_dict(ckpt["model"], strict=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fixed-bool version: posterior(text)=ON, encoder(text)=OFF, fuse(text)=OFF, fuse(x-skip)=ON."
    )
    ...
    # ---- using_clinical switches ----
    parser.add_argument('--no_text', action='store_true', help='Disable clinical note features (time-series only).')
    parser.add_argument('--no_meta_bags', action='store_true', help='Do not load/use meta_bags even if present.')
    parser.add_argument('--decay_lambda', type=float, default=CFG.decay_lambda, help='Exponential time-decay lambda for z_t aggregation (paper best ~0.01).')

    parser.add_argument("--device", type=str, default=CFG.device)
    parser.add_argument("--seed", type=int, default=CFG.seed)
    parser.add_argument("--epochs", type=int, default=CFG.epochs, help="(deprecated) Alias for --outer_epochs when --outer_epochs is not set.")
    parser.add_argument("--inner_epochs", type=int, default=None, help="Max epochs for inner-fold training during Optuna. If omitted, uses CFG.inner_epochs.")
    parser.add_argument("--outer_epochs", type=int, default=None, help="Max epochs for outer-fold final refit training. If omitted, uses --epochs.")
    ...
    # ---- stage-specific max epochs ----
    # We keep --epochs for backward compatibility, but allow separate inner/outer epochs.
    outer_epochs = int(args.outer_epochs) if args.outer_epochs is not None else int(args.epochs)
    inner_epochs = int(args.inner_epochs) if args.inner_epochs is not None else int(CFG.inner_epochs)
    if inner_epochs <= 0:
    ...
    cfg = CFG(
        ...
        use_text=bool(not args.no_text),
        meta_bags_on=bool(not args.no_meta_bags),
    ...
    # ---- using_clinical workflow: keep user-specified switches ----
    if not cfg.use_text:
        cfg.meta_bags_on = False
    cfg.text_source = str(cfg.text_source or 'raw').lower()
    if cfg.text_source not in ('raw','gru'):
