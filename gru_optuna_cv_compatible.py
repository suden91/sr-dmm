...
class CFG:
    ...
    params_json: str = ""
    # Load per-fold best params from a prior run (e.g., {root}/fold_k/optuna_best_params_tune7.json)
    best_params_root: str = ""
    best_params_filename: str = "optuna_best_params_tune7.json"
...
class MRUJointModel(nn.Module):
    """
    GRU-only, Optuna-param compatible variant.

    Goals:
      - Keep the *data pipeline* and *Optuna param JSON keys* compatible with gru_optuna_cv_compatible.py
      - Remove DMM latent-state inference (no z sampling, no recon/KL/distill in loss)
      - Still allow `z_dim` to be used as an extra learned representation (projection from GRU hidden)
        so existing params_json that contains `z_dim` remains meaningful.

    Notes:
      - `posterior_txt_drop_p` is interpreted as *encoder text dropout* for compatibility.
      - recon/kl/distill are returned as 0.0 so the training loop/logging code can remain unchanged.
    """
    ...
    def __init__(self, Dx: int, Du: int, De: int, Dt: int, Ds: int, cfg: CFG, meta_bags: Optional[MetaBagsTorch]):
        ...
        # ---- static embedding (same interface as DMMCore.s_mlp) ----
        self.s_mlp = None
        if bool(getattr(cfg, "death_use_static", True)) and Ds > 0:
            self.s_mlp = nn.Sequential(
                nn.Linear(Ds, cfg.s_emb_dim),
                nn.ReLU(),
                nn.Dropout(cfg.dropout),
        ...
        self.sdim = int(cfg.s_emb_dim) if (self.s_mlp is not None) else 0

        # ---- text feature branches (same as original) ----
        self.txt_gru = None
        ...
        # ---- encoder GRU (replaces DMM encoder) ----
        enc_txt_dim = 0
        ...
        enc_in_dim = int(Dx) + int(Dx) + int(Du) + int(De) + int(enc_txt_dim)  # [x, x_mask, u, e, txt_enc]
        self.enc_gru_fwd = nn.GRU(
            input_size=int(enc_in_dim),
            hidden_size=int(cfg.enc_hidden),
            num_layers=int(cfg.enc_layers),
        ...
        # ---- optional "z_dim" projection for compatibility ----
        self.z_dim = int(getattr(cfg, "z_dim", 0) or 0)
        self.z_proj = None
        if self.z_dim > 0:
            self.z_proj = nn.Sequential(
                nn.Linear(int(cfg.enc_hidden), int(self.z_dim)),
                nn.ReLU(),
                nn.Dropout(cfg.dropout),
            )

        # ---- fuse head input dimension (mirrors original) ----
        feat_dim = int(cfg.enc_hidden) + int(self.z_dim)
        if bool(getattr(cfg, "death_use_static", True)) and self.sdim > 0:
            feat_dim += int(self.sdim)

        fuse_in = int(feat_dim)
    ...
    def _prepare_text_features(
        ...
        return txt_enc, None, feat_t, txt_present
    ...
    def forward_logits(self, batch: Dict[str, torch.Tensor], sample_latent: bool) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Forward for evaluation/inference.
        ...
        # ---- static embedding ----
        if self.s_mlp is not None:
            s_emb = self.s_mlp(s)  # (B, s_emb_dim)
        else:
            s_emb = x.new_zeros((x.size(0), 0))

        # ---- text encoder injection ----
        txt_enc, _txt_post, feat_t, _txt_present = self._prepare_text_features(batch)
        if txt_enc is None:
            enc_in = torch.cat([x, xm, u, e], dim=-1)
        else:
            enc_in = torch.cat([x, xm, u, e, txt_enc], dim=-1)

        h_fwd, _ = self.enc_gru_fwd(enc_in)  # (B,T,H)

        # ---- optional z projection ----
        if self.z_proj is not None:
            z_feat = self.z_proj(h_fwd)  # (B,T,z_dim)
        else:
            z_feat = x.new_zeros((x.size(0), x.size(1), 0))

        feat_parts = [h_fwd, z_feat]
        if bool(getattr(self.cfg, "death_use_static", True)) and s_emb.size(1) > 0:
            feat_parts.append(s_emb.unsqueeze(1).expand(x.size(0), x.size(1), s_emb.size(1)))
        feat_v = torch.cat(feat_parts, dim=-1) if len(feat_parts) > 1 else feat_parts[0]
    ...
    def forward(self, batch: Dict[str, torch.Tensor], bce_loss_fn: nn.Module, sample_latent: bool) -> Dict[str, torch.Tensor]:
        """
        Train forward:
          - Only supervised death loss is used.
          - recon/kl/distill are always 0 (returned for compatibility with existing training loop).
        """
        logits, _r, _k = self.forward_logits(batch, sample_latent=sample_latent)
        ...
        total = float(self.cfg.lambda_death) * death
...
def apply_trial_params(cfg, params: dict):
    ...
    # Compatibility: posterior_txt_drop_p is interpreted as encoder text dropout in GRU-only model.
    p_post = _get("posterior_txt_drop_p", float, cfg.posterior_txt_drop_p)
    p_enc  = _get("encoder_txt_drop_p", float, p_post)
    cfg.posterior_txt_drop_p = float(p_post)
    cfg.encoder_txt_drop_p   = float(p_enc)
...
def load_best_params_for_fold(best_params_root: str, fold_id: int, filename: str = "optuna_best_params_tune7.json") -> Dict[str, Any]:
    """Load per-fold best params.

    Accepts:
    - A JSON file path: either a dict of params, or a dict containing fold-specific keys (fold_k).
    - A directory root: searches under {root}/fold_{fold_id}/ for common filenames.
    """
    best_params_root = os.path.abspath(str(best_params_root))

    # Case 1) best_params_root is a JSON file
...
def run_outer_fold_optuna_nested(fold_dir: str, cfg: CFG) -> Dict[str, Any]:
    ...
    # ---- optional: bypass Optuna using fixed params (per-fold best_params_root OR a single params_json) ----
    fixed_params = None
    fixed_best_value = float("nan")

    # (A) per-fold params from best_params_root (preferred if provided)
    bpr = str(getattr(cfg, "best_params_root", "") or "")
    bpf = str(getattr(cfg, "best_params_filename", "optuna_best_params_tune7.json") or "optuna_best_params_tune7.json")
    if bpr:
        fixed_params = load_best_params_for_fold(bpr, fold_id, bpf)
        print(f"[FOLD {fold_id}] [BEST_PARAMS_ROOT] using per-fold params from: {os.path.abspath(bpr)} (file={bpf})")

    # (B) single params_json applied to all folds (fallback)
    if fixed_params is None and getattr(cfg, "params_json", ""):
        pj = str(getattr(cfg, "params_json", "") or "")
        if pj and os.path.exists(pj):
            with open(pj, "r") as f:
                fixed_params = json.load(f) or {}
            if not isinstance(fixed_params, dict):
                fixed_params = {}
            print(f"[FOLD {fold_id}] [PARAMS_JSON] using fixed params from: {pj}")
    if fixed_params is None:
        ...
        fixed_best_value = float(best.value) if best.value is not None else float("nan")
    else:
        best_params = dict(fixed_params)

    # txt_layers is fixed (not suggested) in many configs, so it may be absent.
    best_params.setdefault("txt_layers", 2)

    # If best_params_root was used, keep a copy for traceability
    if fixed_params is not None and str(getattr(cfg, "best_params_root", "") or ""):
        try:
    ...
    if fixed_params is None:
        print(f"[FOLD {fold_id}] ✅ Optuna best value={fixed_best_value:.4f} | params saved -> optuna_best_params_tune7.json")
    else:
        print(f"[FOLD {fold_id}] ✅ Fixed params loaded | params saved -> optuna_best_params_tune7.json")
    ...
    summary = {
        ...
        "optuna_best_value_inner_mean_pr_auc": float(fixed_best_value),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GRU-only (no recon/KL) version, compatible with Optuna params JSON keys from the DMM scripts."
    )
    ...
    parser.add_argument("--best_params_root", type=str, default="",
                        help="If set, load per-fold best params from this root (e.g., root/fold_k/optuna_best_params_tune7.json). Overrides --params_json.")
    parser.add_argument("--best_params_filename", type=str, default="optuna_best_params_tune7.json",
                        help="Filename to look for under each fold directory inside --best_params_root.")
    ...
    cfg = CFG(
        ...
        params_json=str(args.params_json or ""),
        best_params_root=str(getattr(args, "best_params_root", "") or ""),
        best_params_filename=str(getattr(args, "best_params_filename", "optuna_best_params_tune7.json") or "optuna_best_params_tune7.json"),
        device=args.device,
        seed=args.seed,
        epochs=args.epochs,
    ...
    cfg.use_smoother_train = False  # GRU-only (no smoother / no recon-KL)
