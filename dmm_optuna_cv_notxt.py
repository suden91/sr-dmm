...
class CFG:
    ...
    # ============================================================
    use_text: bool = False
    meta_bags_on: bool = False
    encoder_use_text: bool = False

    text_source: str = "raw"  # {"raw","gru"} 중 raw로 고정

    # posterior(q) text OFF
    posterior_use_text: bool = False
    posterior_txt_use_mask: bool = False
    posterior_txt_detach: bool = False
    posterior_txt_drop_p: float = 0.0
...
def collate_pad(batch: List[Dict[str, torch.Tensor]], pad_to: int) -> Dict[str, torch.Tensor]:
    """
    Pad/truncate variable-length sequences in a batch to `pad_to`.

    NOTE (important):
      - Avoid torch.nn.functional.pad because it errors when a non-padded dimension has size 0
        (e.g., Dt==0 when text is disabled). This implementation supports last-dim == 0 safely.
      - If any sequence is longer than pad_to, it is truncated and seq_mask/lengths are clamped.
    """
    pad_to = int(pad_to)
    lengths = torch.stack([b["length"] for b in batch]).long()
    lengths_clamped = torch.clamp(lengths, max=pad_to)

    out: Dict[str, torch.Tensor] = {}

    def _pad_3d(t: torch.Tensor, fill: float = 0.0) -> torch.Tensor:
        # t: (T, D)
        if t.dim() != 2:
            raise ValueError(f"Expected 2D tensor (T,D), got shape={tuple(t.shape)}")
        T, D = int(t.size(0)), int(t.size(1))
        if T >= pad_to:
            return t[:pad_to].unsqueeze(0)  # (1, pad_to, D)
        # T < pad_to
        z = t.new_full((pad_to, D), fill)
        if T > 0:
            z[:T] = t
        return z.unsqueeze(0)

    def _pad_2d(t: torch.Tensor, fill: int = 0) -> torch.Tensor:
        # t: (T,)
        if t.dim() != 1:
            raise ValueError(f"Expected 1D tensor (T,), got shape={tuple(t.shape)}")
        T = int(t.size(0))
        if T >= pad_to:
            return t[:pad_to].unsqueeze(0)  # (1, pad_to)
        z = t.new_full((pad_to,), fill)
        if T > 0:
            z[:T] = t
        return z.unsqueeze(0)

    # 3D keys (per item: (T, Dk))
    keys_3d = ["x", "x_mask", "u", "e", "txt"]
    for k in keys_3d:
        out[k] = torch.cat([_pad_3d(b[k], fill=0.0) for b in batch], dim=0)

    # row_id: (T,) with fill=-1
    out["row_id"] = torch.cat([_pad_2d(b["row_id"].long(), fill=-1) for b in batch], dim=0)

    # y / masks: (T,) with fill=0
    for k in ["y", "y_mask", "time_mask"]:
        out[k] = torch.cat([_pad_2d(b[k], fill=0) for b in batch], dim=0)

    # static: (Ds,)
    out["s"] = torch.stack([b["s"] for b in batch], dim=0)

    B = len(batch)
    ar = torch.arange(pad_to).unsqueeze(0).expand(B, pad_to)
    out["seq_mask"] = (ar < lengths_clamped.unsqueeze(1)).float()
    out["lengths"] = lengths_clamped
...
def run_outer_fold_optuna_nested(fold_dir: str, cfg: CFG) -> Dict[str, Any]:
    ...
    # If this fold was partially/fully optimized before (e.g., a crash after Optuna),
    # avoid running extra trials beyond the requested budget.
    timeout = int(cfg.optuna_timeout_sec) if int(cfg.optuna_timeout_sec) > 0 else None
    requested = int(cfg.n_trials)
    try:
        existing = len(study.get_trials(deepcopy=False))
    except Exception:
        existing = len(study.trials)
    to_run = max(0, requested - existing)
    if to_run <= 0:
        print(f"[FOLD {fold_id}] Optuna trials already present: {existing} (requested={requested}) -> skip optimize")
    else:
        print(f"[FOLD {fold_id}] Optuna will run {to_run} more trials (existing={existing}, requested={requested})")
    ...
    # NOTE: this *no-text* script must never re-enable text modules at eval-time,
    # otherwise the checkpoint (trained with Dt=0) will not match the model.
    cfg_loaded.use_text = False
    cfg_loaded.meta_bags_on = False
    cfg_loaded.text_source = "raw"
    cfg_loaded.encoder_use_text = False
    cfg_loaded.encoder_txt_use_mask = False
    cfg_loaded.posterior_use_text = False
    cfg_loaded.posterior_txt_use_mask = False
    cfg_loaded.fuse_use_text = False
    # keep x-skip / other knobs as stored in ckpt["cfg"], but ensure they are valid
    cfg_loaded.fuse_use_x = bool(getattr(cfg_loaded, "fuse_use_x", True))
    cfg_loaded.fuse_x_use_mask = bool(getattr(cfg_loaded, "fuse_x_use_mask", True))
    cfg_loaded.scale_txt = False
    cfg_loaded.amp = bool(getattr(cfg_loaded, "amp", True))
...
if __name__ == "__main__":
    ...
    # Text is completely OFF in this variant.
    cfg.use_text = False
    cfg.meta_bags_on = False
    cfg.text_source = "raw"
    cfg.encoder_use_text = False
    cfg.encoder_txt_use_mask = False

    cfg.posterior_use_text = False
    cfg.posterior_txt_use_mask = False
    cfg.posterior_txt_detach = False
    cfg.posterior_txt_drop_p = 0.0
